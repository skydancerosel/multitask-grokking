#!/usr/bin/env python3
"""
Spectral analysis of modadd_modmul (two-task) grokking checkpoints.
...
Two-task model: shared trunk, 2 heads → (x+y)%p, (x*y)%p
"""
...
SEEDS = [42, 137, 2024]
TASK_NAMES = ["add", "mul"]
TASK_COLORS = {"add": "#e41a1c", "mul": "#377eb8"}
...
def load_run(wd, seed):
    """Load a multitask checkpoint. Handles the file naming conventions."""
    # Try different naming patterns
    candidates = []
    if wd == 1.0:
        candidates.append(f"multitask_s{seed}.pt")
    if wd == 0.0:
        candidates.append(f"multitask_nowd_s{seed}.pt")

    # wd-specific names: multitask_wd01_s42.pt = wd=0.1
    wd_str = f"{wd:.1f}".replace(".", "")  # 0.1 → 01, 0.5 → 05
    candidates.append(f"multitask_wd{wd_str}_s{seed}.pt")

    # Also try integer: wd1 → multitask_wd1_s42.pt
    if wd == int(wd):
        candidates.append(f"multitask_wd{int(wd)}_s{seed}.pt")
    ...
    print(f"  [SKIP] No file for wd={wd}, seed={seed} (tried: {candidates})")
    return None


def extract_grok_steps(data):
    """Extract per-task grok steps from the checkpoint."""
    grok_steps = {}
    # modadd_modmul stores grok info differently: grokked_add, grok_step_add, etc.
    for task in TASK_NAMES:
        gs = data.get(f"grok_step_{task}")
        if gs is not None:
            grok_steps[task] = gs
    return grok_steps
...
def plot_master_timeseries(all_data, save_path):
    ...
    fig.suptitle("ModAdd+ModMul: Weight SVD gaps & commutators (wd=1.0, 3 seeds)",
                 fontsize=14, y=1.01)
    plt.tight_layout()
...
def plot_narrative_test(all_data, save_path):
    ...
    for row, (seed, d) in enumerate(sorted(all_data.items())):
        ...
        ax.plot(steps_m, avg_test, color="#17becf", lw=1.5, ls=":",
                label="Avg test acc")
    ...
    fig.suptitle("ModAdd+ModMul: Testing spectral symmetry-breaking narrative\n"
...
def plot_hero_portrait(all_data, save_path, hero_seed=42):
    ...
    fig.suptitle(f"ModAdd+ModMul phase portrait  (seed {hero_seed}, layer 0)\n"
                 f"$x = \\sigma_1 - \\sigma_2$  vs  "
                 f"$y = \\|[W_Q, W_K]\\|_F$\n"
                 f"add@{grok_steps.get('add','?')}, "
...
def plot_grid_portrait(all_data, save_path):
    ...
    fig.suptitle("ModAdd+ModMul phase portraits: 3 seeds (wd=1.0, smoothed)",
                 fontsize=14, y=1.01)
    plt.tight_layout()
...
def plot_grok_vs_control(all_data_grok, save_path, hero_seed=42):
    ...
    for col, (wd, label, ax) in enumerate([
        ...
        if wd == 1.0:
            ...
            steps_sv = sv["steps"]
            grok_steps = extract_grok_steps(ctrl)
    ...
    fig.suptitle("ModAdd+ModMul: grokking vs memorizing in phase space\n"
...
def plot_svd_grok_vs_control(all_data_grok, save_path, hero_seed=42):
    ...
    fig.suptitle(f"ModAdd+ModMul SVD dynamics: grokking vs control (seed={hero_seed})",
                 fontsize=13)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_wd_sweep_portraits(save_path, hero_seed=42):
    ...
    for idx, wd in enumerate(wds):
        ...
        steps_sv = sv["steps"]
        grok_steps = extract_grok_steps(data)
        ...
        ax.set_title(f"wd={wd}  ({n_grokked}/{len(TASK_NAMES)} grokked)", fontsize=11)

    fig.suptitle(f"ModAdd+ModMul phase portraits across weight decay (seed={hero_seed})",
                 fontsize=14, y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=175, bbox_inches="tight")
    plt.close()
...
def print_timing_analysis(all_data):
    print(f"\n{'='*72}")
    print("MODADD+MODMUL TIMING ANALYSIS")
    print(f"{'='*72}")
...
def main():
    print("=" * 72)
    print("MODADD+MODMUL SPECTRAL ANALYSIS")
    print("=" * 72)
